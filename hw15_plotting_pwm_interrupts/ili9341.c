...
void SPI1_init() {
  ...
  TRISAbits.TRISA0 = 0;     // set A0 as an output (manual CS2)
  CS = 1; // CS starts high (inactive)
  CS2 = 1;  // CS2 starts high (inactive)
...
void LCD_command(unsigned char com) {
    DC = 0; // DC --> The LCD requires an additional pin to know if a byte is a command or represents data
    spi_io(com);
    DC = 1; // DC
}
...
void LCD_data16(unsigned short dat) {  // to send two bytes at a time
    spi_io(dat>>8);
    spi_io(dat);
}

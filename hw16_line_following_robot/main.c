...
int main() {
    ...
    while(1) {
        ...
        */
        ...
        // vertical read
        int c = ov7670_count_vert(d);   // c = count
        sprintf(message, "c = %d   ",c);
...
//            speed = 2399 - (2399/(c/2/2))*e; // when the com is all the way over, the motor is all off; 
            speed = 2399 - Kp * e;   // so speed goes down
            if(speed > 2399){
                speed = 2399;
            }
            if(speed < 0){
            ...
            OC4RS = 2399 *.2;   // there's a wheel turning at max speed at all times
            OC1RS = speed *.2;
            eprev = e;
        }
        else {
            e = com - c/2/2;  // switch order so e remains positive
            edot = e - eprev;
            eint = eint + e;
//            speed = 2399 - (2399/(c/2/2))*e; // when the com is all the way over, the motor is all off
            speed = 2399 - Kp * e;   // so speed goes down
